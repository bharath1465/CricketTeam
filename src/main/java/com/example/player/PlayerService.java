...
public class PlayerService implements PlayerRepository {
    ...
    public Player getPlayerId(int playerId){
        Player teamPlayer = team.get(playerId);
        if(teamPlayer == null) throw new ResponseStatusException(HttpStatus.NOT_FOUND);
        return teamPlayer;
    }

    @Override
    public Player addPlayer(Player teamPlayer) {
        teamPlayer.setId(uniquePlayerId);
        team.put(uniquePlayerId, teamPlayer);
        uniquePlayerId += 1;
    ...
    public Player updatePlayer(int playerId,Player teamPlayer){
        ...
        if (teamPlayer.getName() != null) {
            existingPlayer.setName(teamPlayer.getName());
        }

        if (teamPlayer.getJerseyNumber() != 0){
            existingPlayer.setJerseyNumber(teamPlayer.getJerseyNumber());
        }
        
        if (teamPlayer.getRole() != null) {
            existingPlayer.setRole(teamPlayer.getRole());
        }
